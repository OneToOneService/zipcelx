...
const CELL_TYPE_TIMESTAMP = 'timestamp';
const validTypes = [CELL_TYPE_STRING, CELL_TYPE_NUMBER, CELL_TYPE_DATE, CELL_TYPE_TIMESTAMP];
...
var generatorDateCell = (index, value, rowIndex) => (`<c r="${generatorCellNumber(index, rowIndex)}" s="1"><v>${escape(value)}</v></c>`);

var generatorTimestampCell = (index, value, rowIndex) => (`<c r="${generatorCellNumber(index, rowIndex)}" s="2"><v>${escape(value)}</v></c>`);
